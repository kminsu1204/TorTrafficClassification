...
class Dataset:
    def __init__(self, sampling_size, test_size):
    ...
    def data_sampling(self, file_name, sampling_size):
        #import pdb;pdb.set_trace()
        n = sum(1 for line in open(file_name))
        samples = random.sample(range(n), n - sampling_size)
        return samples

#    def read_tor_csv_file(self, file_name, sampling_size):
#        tor_flags = tf.app.flags
#        tor_FLAGS = tor_flags.FLAGS
#        tor_flags.DEFINE_string('Tor_dir', 'Tor', 'Tor Input Directory') # return Tor\\file_name
#        csv_file = os.path.join(tor_FLAGS.Tor_dir, file_name)
#        return pd.read_csv(csv_file, delimiter=',', dtype=np.float32, header=None, skiprows=self.data_sampling(file_name, sampling_size))
    
    def read_csv_file(self, file_name, sampling_size):
        return pd.read_csv(file_name, delimiter=',', dtype=np.float32, header=None, skiprows=self.data_sampling(file_name, sampling_size))
    ...
    def _get_data(self):
        ...
        AUDIO_spotifygateway = self.read_csv_file(r'Tor/AUDIO_spotifygateway.csv', self.sampling_size)  # 35485 (number of packets)
        AUDIO_tor_spotify = self.read_csv_file(r'Tor/AUDIO_tor_spotify.csv', self.sampling_size)    # 35493
        AUDIO_tor_spotify2 = self.read_csv_file(r'Tor/AUDIO_tor_spotify2.csv', self.sampling_size)  # 159018
        
        print('Reading Browsing related csv files...')
        BROWSING_gate_SSL_Browsing = self.read_csv_file(r'Tor/BROWSING_gate_SSL_Browsing.csv', self.sampling_size)  # 117531
        BROWSING_ssl_browsing_gateway = self.read_csv_file(r'Tor/BROWSING_ssl_browsing_gateway.csv', self.sampling_size)    # 41235
        BROWSING_tor_browsing_ara = self.read_csv_file(r'Tor/BROWSING_tor_browsing_ara.csv', self.sampling_size)    # 99878
        BROWSING_tor_browsing_ger = self.read_csv_file(r'Tor/BROWSING_tor_browsing_ger.csv', self.sampling_size)    # 154079
        BROWSING_tor_browsing_mam = self.read_csv_file(r'Tor/BROWSING_tor_browsing_mam.csv', self.sampling_size)    # 170612
        BROWSING_tor_browsing_mam2 = self.read_csv_file(r'Tor/BROWSING_tor_browsing_mam2.csv', self.sampling_size)  # 43062
        
        print('Reading Chat related csv files...')
        CHAT_aimchatgateway = self.read_csv_file(r'Tor/CHAT_aimchatgateway.csv', self.sampling_size)    # 227
        CHAT_facebookchatgateway = self.read_csv_file(r'Tor/CHAT_facebookchatgateway.csv', self.sampling_size)  # 5406
        CHAT_gate_AIM_chat = self.read_csv_file(r'Tor/CHAT_gate_AIM_chat.csv', self.sampling_size)  # 580
        CHAT_gate_facebook_chat = self.read_csv_file(r'Tor/CHAT_gate_facebook_chat.csv', self.sampling_size)    # 2902
        CHAT_gate_hangout_chat = self.read_csv_file(r'Tor/CHAT_gate_hangout_chat.csv', self.sampling_size)  # 3566
        CHAT_gate_ICQ_chat = self.read_csv_file(r'Tor/CHAT_gate_ICQ_chat.csv', self.sampling_size)  # 417
        CHAT_gate_skype_chat = self.read_csv_file(r'Tor/CHAT_gate_skype_chat.csv', self.sampling_size)  # 1907
        CHAT_hangoutschatgateway = self.read_csv_file(r'Tor/CHAT_hangoutschatgateway.csv', self.sampling_size)  # 2948
        CHAT_icqchatgateway = self.read_csv_file(r'Tor/CHAT_icqchatgateway.csv', self.sampling_size)    # 534
        CHAT_skypechatgateway = self.read_csv_file(r'Tor/CHAT_skypechatgateway.csv', self.sampling_size)    # 2135
        
        print('Reading File transfer related csv files...')
        FILE_TRANSFER_gate_FTP_transfer = self.read_csv_file(r'Tor/FILE-TRANSFER_gate_FTP_transfer.csv', self.sampling_size)    # 2952252
        FILE_TRANSFER_gate_SFTP_filetransfer = self.read_csv_file(r'Tor/FILE-TRANSFER_gate_SFTP_filetransfer.csv', self.sampling_size)  # 929868
        FILE_TRANSFER_tor_skype_transfer = self.read_csv_file(r'Tor/FILE-TRANSFER_tor_skype_transfer.csv', self.sampling_size)  # 123862
        
        print('Reading Mail related csv files...')
        MAIL_gate_Email_IMAP_filetransfer = self.read_csv_file(r'Tor/MAIL_gate_Email_IMAP_filetransfer.csv', self.sampling_size)    # 95748
        MAIL_gate_POP_filetransfer = self.read_csv_file(r'Tor/MAIL_gate_POP_filetransfer.csv', self.sampling_size)  # 77794
        MAIL_Gateway_Thunderbird_Imap = self.read_csv_file(r'Tor/MAIL_Gateway_Thunderbird_Imap.csv', self.sampling_size)    # 133752
        MAIL_Gateway_Thunderbird_POP = self.read_csv_file(r'Tor/MAIL_Gateway_Thunderbird_POP.csv', self.sampling_size)  # 84118
        
        print('Reading P2P related csv files...')
        P2P_tor_p2p_multipleSpeed = self.read_csv_file(r'Tor/P2P_tor_p2p_multipleSpeed.csv', self.sampling_size)    # 1342497
        P2P_tor_p2p_vuze = self.read_csv_file(r'Tor/P2P_tor_p2p_vuze.csv', self.sampling_size)  # 1448963
        
        print('Reading Video related csv files...')
        VIDEO_Vimeo_Gateway = self.read_csv_file(r'Tor/VIDEO_Vimeo_Gateway.csv', self.sampling_size)    # 1355043
        VIDEO_Youtube_Flash_Gateway = self.read_csv_file(r'Tor/VIDEO_Youtube_Flash_Gateway.csv', self.sampling_size)    # 502737
        VIDEO_Youtube_HTML5_Gateway = self.read_csv_file(r'Tor/VIDEO_Youtube_HTML5_Gateway.csv', self.sampling_size)    # 270387
        
        print('Reading VoIP related csv files...')
        VOIP_Facebook_Voice_Gateway = self.read_csv_file(r'Tor/VOIP_Facebook_Voice_Gateway.csv', self.sampling_size)    # 317194
        VOIP_gate_facebook_Audio = self.read_csv_file(r'Tor/VOIP_gate_facebook_Audio.csv', self.sampling_size)  # 220995
        VOIP_gate_hangout_audio = self.read_csv_file(r'Tor/VOIP_gate_hangout_audio.csv', self.sampling_size)    # 266986
        VOIP_gate_Skype_Audio = self.read_csv_file(r'Tor/VOIP_gate_Skype_Audio.csv', self.sampling_size)    # 213763
        VOIP_Hangouts_voice_Gateway = self.read_csv_file(r'Tor/VOIP_Hangouts_voice_Gateway.csv', self.sampling_size)    # 312869
        VOIP_Skype_Voice_Gateway = self.read_csv_file(r'Tor/VOIP_Skype_Voice_Gateway.csv', self.sampling_size)  # 217814

        print('Reading nonTor csv files...')
        print('Reading Audio related csv files...')
        facebook_Audio = self.read_csv_file(r'nonTor/facebook_Audio.csv', self.sampling_size)   # 218800
        Hangout_Audio = self.read_csv_file(r'nonTor/Hangout_Audio.csv', self.sampling_size) # 244039
        Skype_Audio = self.read_csv_file(r'nonTor/Skype_Audio.csv', self.sampling_size) # 199036
        spotify = self.read_csv_file(r'nonTor/spotify.csv', self.sampling_size) # 42947
        spotify2 = self.read_csv_file(r'nonTor/spotify2.csv', self.sampling_size)   # 165372
        spotifyAndrew = self.read_csv_file(r'nonTor/spotifyAndrew.csv', self.sampling_size) # 34480
       
        print('Reading Browsing related csv files...')
        browsing = self.read_csv_file(r'nonTor/browsing.csv', self.sampling_size)   # 219535
        browsing_ara = self.read_csv_file(r'nonTor/browsing_ara.csv', self.sampling_size)   # 118384
        browsing_ara2 = self.read_csv_file(r'nonTor/browsing_ara2.csv', self.sampling_size) # 88520
        browsing_ger = self.read_csv_file(r'nonTor/browsing_ger.csv', self.sampling_size)   # 185055
        browsing2 = self.read_csv_file(r'nonTor/browsing2.csv', self.sampling_size) # 54933
        ssl = self.read_csv_file(r'nonTor/ssl.csv', self.sampling_size) # 52855
        SSL_Browsing = self.read_csv_file(r'nonTor/SSL_Browsing.csv', self.sampling_size)   # 97926
        
        print('Reading Chat related csv files...')
        AIM_Chat = self.read_csv_file(r'nonTor/AIM_Chat.csv', self.sampling_size)   # 526
        aimchat = self.read_csv_file(r'nonTor/aimchat.csv', self.sampling_size) # 190
        facebook_chat = self.read_csv_file(r'nonTor/facebook_chat.csv', self.sampling_size) # 2969
        facebookchat = self.read_csv_file(r'nonTor/facebookchat.csv', self.sampling_size)   # 5814
        hangout_chat = self.read_csv_file(r'nonTor/hangout_chat.csv', self.sampling_size)   # 3510
        hangoutschat = self.read_csv_file(r'nonTor/hangoutschat.csv', self.sampling_size)   # 2994
        ICQ_Chat = self.read_csv_file(r'nonTor/ICQ_Chat.csv', self.sampling_size)   # 386
        icqchat = self.read_csv_file(r'nonTor/icqchat.csv', self.sampling_size) # 532
        skype_chat = self.read_csv_file(r'nonTor/skype_chat.csv', self.sampling_size)   # 2121
        skypechat = self.read_csv_file(r'nonTor/skypechat.csv', self.sampling_size) # 2415

        print('Reading File transfer related csv files...')
        FTP_filetransfer = self.read_csv_file(r'nonTor/FTP_filetransfer.csv', self.sampling_size)   # 1710680
        SFTP_filetransfer = self.read_csv_file(r'nonTor/SFTP_filetransfer.csv', self.sampling_size) # 526882
        skype_transfer = self.read_csv_file(r'nonTor/skype_transfer.csv', self.sampling_size)   # 143083

        print('Reading Mail related csv files...')
        Email_IMAP_filetransfer = self.read_csv_file(r'nonTor/Email_IMAP_filetransfer.csv', self.sampling_size) # 71235
        POP_filetransfer = self.read_csv_file(r'nonTor/POP_filetransfer.csv', self.sampling_size)   # 58913
        Workstation_Thunderbird_Imap = self.read_csv_file(r'nonTor/Workstation_Thunderbird_Imap.csv', self.sampling_size)   # 81653
        Workstation_Thunderbird_POP = self.read_csv_file(r'nonTor/Workstation_Thunderbird_POP.csv', self.sampling_size) # 113529
        
        print('Reading P2P related csv files...')
        p2p_multipleSpeed = self.read_csv_file(r'nonTor/p2p_multipleSpeed.csv', self.sampling_size) # 1328955
        p2p_vuze = self.read_csv_file(r'nonTor/p2p_vuze.csv', self.sampling_size)   # 1109575

        print('Reading Video related csv files...')
        Vimeo_Workstation = self.read_csv_file(r'nonTor/Vimeo_Workstation.csv', self.sampling_size) # 984223
        Youtube_Flash_Workstation = self.read_csv_file(r'nonTor/Youtube_Flash_Workstation.csv', self.sampling_size) # 494769
        Youtube_HTML5_Workstation = self.read_csv_file(r'nonTor/Youtube_HTML5_Workstation.csv', self.sampling_size) # 265313

        print('Reading VoIP related csv files...')
        Facebook_Voice_Workstation = self.read_csv_file(r'nonTor/Facebook_Voice_Workstation.csv', self.sampling_size)   # 253691
        Hangouts_voice_Workstation = self.read_csv_file(r'nonTor/Hangouts_voice_Workstation.csv', self.sampling_size)   # 261894
        Skype_Voice_Workstation = self.read_csv_file(r'nonTor/Skype_Voice_Workstation.csv', self.sampling_size) # 194319
